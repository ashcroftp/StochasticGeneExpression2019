...
rateFunc <- function(x, params, t) {
    ...
    bound.fraction <- x[["target.dimer"]]/(x[["target.protein"]] + x[["target.dimer"]])
    ...
    rates <- with(
        ...
        c(
            ...
            # CELL DEATH (depends on NUMBER of bound TARGETS)
            #((
            #    (rate.maxGrowth - rate.minGrowth) * (x[["target.dimer"]] / MIC) ^ kappa
            #) / (
            #    (x[["target.dimer"]] / MIC) ^ kappa - (rate.minGrowth / rate.maxGrowth)
            #)) * x[["alive"]]
            # # CELL DEATH (depends on FRACTION of bound TARGETS)
            # ((
            #     (rate.maxGrowth - rate.minGrowth) * ( bound.fraction / MIC.fraction) ^ kappa
            # ) / (
            #     ( bound.fraction / MIC.fraction) ^ kappa - (rate.minGrowth / rate.maxGrowth)
            # )) * x[["alive"]]
            # CELL DEATH (depends on FRACTION of bound TARGETS) [from Regoes 2004]
            # log(10) * ((
            #     (rate.maxGrowth - rate.minGrowth) * (bound.fraction / MIC.fraction) ^ kappa
            # ) / (
            #     (bound.fraction / MIC.fraction) ^ kappa - (rate.minGrowth / rate.maxGrowth)
            # )) * x[["alive"]]
            log(10) * ((
                     (((rate.maxGrowth - rate.minGrowth) * ((1/MIC.fraction)^kappa - 1))/((1/MIC.fraction)^kappa - (rate.maxGrowth - rate.minGrowth) / rate.maxGrowth)) * (bound.fraction / MIC.fraction) ^ kappa
                 ) / (
                     (bound.fraction / MIC.fraction) ^ kappa - (1 - (((rate.maxGrowth - rate.minGrowth) * ((1/MIC.fraction)^kappa - 1))/(rate.maxGrowth * (1/MIC.fraction)^kappa - (rate.maxGrowth - rate.minGrowth))))
            )) * x[["alive"]]
        )
    )
...
#' This could be \code{"tetracycline"}, \code{"rifampicin"}, or \code{"ciprofloxacin"}.
...
loadParameters <- function(drug = "ciprofloxacin") {
    ...
    params <- list(
        ...
        rate.target.protein.translation = drug.targets / cell.generationTime,
        # Want average target density to be constant
        # EFFLUX parameters
        ...
        sd.generationTime = 0.05, # In percent
        # Mutant effects
...
#' Parameters of individual cells, which are mutation-dependent and use random numbers.
#' 
...
#' This can be \code{"WT"}, \code{"Reg"}, or \code{"Struct"}
#' @return (list): A named list in the same format of \code{loadParameters()}, but tailored to each cell.
#' @examples cellParameters(basic.params, mutation="Reg")
cellParameters <- function(basic.params, mutation = "WT", t = 0) {
    ...
    ## Generation time for each cell has a (uncorrelated) noise element
    ...
    ) # Normally-distributed generation times
...
#' \code{ssa.adaptivetau()} uses the tau-leaping framework, and leads to faster (but apporximate) simulations.
...
cellSSA <- function(cell.params, initial.condition, sim.t) {
    # Perform the SSA from t=0 until t=sim.t
    #cell.data <- ssa.exact(initial.condition, stoich, rateFunc, cell.params, sim.t)
    cell.data <- ssa.adaptivetau(initial.condition, stoich, rateFunc, cell.params, sim.t, tl.params = list(maxtau = 0.1, epsilon = 0.01))
...
cellDivide <- function(parent.cell.data) {
  ...
  # The lines commented out below are options for evenly distributed proteins with and without 5% sd. 
    ...
    # EFFLUX
    efflux.DNA.1        <- parent.cell.data[["efflux.DNA"]]
    ...
    efflux.protein.1    <- rbinom(1, parent.cell.data[["efflux.protein"]], 0.5)
    ...
    efflux.dimer.1      <- rbinom(1, parent.cell.data[["efflux.dimer"]], 0.5)
    ...
    # Add daughters to dataframe list, and return
...
#' @return cell.data.list (list): List of dataframes of each cells dynamics (protein number, drug dynamics, alive/dead status, etc. over time)
#' @return family.tree (dataframe): A dataframe of cells fundamental properties (ID, parent, birth time, end of life, did it die, is it a mutant...)
#' @examples simulateCells(basic.params, initial.cell.data, initial.cell.types=c("WT","WT","Reg"), t.max=100)
simulateCells <- function(basic.params,
    ...
    ## Cell parameter list
    # Each cell has it's own parameters, which we store as a list
    ...
    while (min.t <= t.max) {
        ...
        if (this.cell.information$alive == 1 & sim.t == this.cell.generation.time) {
            ...
            daughters.data.list <- cellDivide(parent.cell.data)
