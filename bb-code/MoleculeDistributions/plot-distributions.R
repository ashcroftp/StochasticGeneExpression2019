#' plot-distributions.R
#' Author: Peter Ashcroft, ETH Zurich

#' A script to investigate distributions of molecules in cells in the absence of antibiotics.
#' We can also compare with some theoretical predictions.
...
data.directory <- "MoleculeDistributions/WT-data/"
cell.data <- read.csv(combined.file(data.directory, "ciprofloxacin_WT"))

#' Get fitted distribution
efflux.range <- c(0:max(cell.data$efflux.protein))
fit <- fitdistr(x = cell.data$efflux.protein, "negative binomial")$estimate
...
plot.list <- list(
    ...
    efflux.mRNA = density.plot(cell.data, "efflux.mRNA", colour = palette_OkabeIto[2]) +
        scale_y_log10(labels = sciFormat),
    efflux.protein = density.plot(cell.data, "efflux.protein", colour = palette_OkabeIto[2]) +
