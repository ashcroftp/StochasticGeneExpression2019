...
runSim <- function(cell.type, output, num.realisations, data.directory, file.id, var.param.index, node.index) {
    ...
    if (output == "survival") {
        ...
        t.max <- 1200
        #' Run simulations within loop, and extract the population size at the end of each simulation
        pop.survival <- DFapply(seq_len(num.realisations), function(run.label) {
            #' Set seed
        ...
        #' Output popluation growth curves of 100 cells, which are sampled every 10 minutes for 1 hour
    ...
    if (length(args) == 1) {
        ...
        if (EULER) {
            ...
            invisible(
                lapply(seq_len(nrow(var.params)), function(parameter.index) {
                    system(
                        paste(
                            paste("bsub -J \"simulate.R", file.id, parameter.index, paste("[1-", num.nodes, "]\"", sep = ""), sep = " "),
            ...
            invisible(
                lapply(seq_len(nrow(var.params)), function(parameter.index) {
                    runSim(cell.type = cell.type, output = output, num.realisations = total.sims, data.directory = data.directory, file.id = file.id, var.param.index = parameter.index, node.index = 0)
                })
            )
        }
        ...
        if (args[2] == "compress") {
            ...
            data <- DFapply(parameter.indices, function(parameter.index) {
                #' Identify the output files, and break the labels into their components
...
profvis(run("Benchmarks/config-WT-survival.R"))